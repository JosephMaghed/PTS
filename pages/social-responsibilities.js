...
export default function SocialResponsibilityPage() {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setCurrentImgIndex((prevIndex) => (prevIndex + 1) % companyData.images.length);
    }, 4000);
  ...
  const handlePrevClick = () => {
    setCurrentImgIndex((prevIndex) => (prevIndex - 1 + companyData.images.length) % companyData.images.length);
  };
  ...
  return (
    <div className="container py-8">
      ...
      />
      <CompanyInfo companyData={{ ...companyData, img: currentImg }} perks={perks} handlePrevClick={handlePrevClick} handleNextClick={handleNextClick} />
      <div className="gallery-navigation right-side-item" style={{float:'right' ,marginRight:"230px"}}>
